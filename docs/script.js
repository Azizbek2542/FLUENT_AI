...
(function () {
  ...
  /* ── Get pixel-perfect center of the SVG icon on screen ── */
  function getIconCenter() {
    // Always read the live SVG that's currently in the DOM
    const svg = themeBtn?.querySelector('svg');
    if (!svg) {
      // Fallback: center of the button itself
      const r = themeBtn?.getBoundingClientRect() ?? { left:0, top:0, width:0, height:0 };
  ...
  /* ── Farthest corner distance from a point ── */
  function maxR(x, y) {
  ...
  async function toggle() {
    ...
    // Read icon center BEFORE any DOM change (icon is still the current one)
