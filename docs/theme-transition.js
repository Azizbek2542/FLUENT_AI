(function () {
  ...
  /*
   * Два разных пути — два разных viewport:
   *
   * ::view-transition (View Transition API) — координаты в LAYOUT viewport
   *   layout viewport = весь экран браузера включая адресную строку
   *   getBoundingClientRect() — в VISUAL viewport (без адресной строки)
   *   Поправка: + visualViewport.offsetTop (= высота адресной строки сверху)
   *
   * position:fixed overlay (fallback) — координаты в VISUAL viewport
   *   getBoundingClientRect() уже в visual viewport
   *   Поправка: не нужна (0)
   */
  function getSVGCenter() {
    ...
    return {
      // Для position:fixed overlay — без поправки (visual viewport)
      visual: { x: Math.round(vcx),        y: Math.round(vcy) },
      // Для ::view-transition — с поправкой на адресную строку (layout viewport)
      layout: { x: Math.round(vcx + offX), y: Math.round(vcy + offY) }
    };
  }
  ...
  async function toggle() {
    ...
    const centers = getSVGCenter();
    const dur     = 600;
    const nextDark = !isDark;

    const vv = window.visualViewport;
    // layout viewport размер
    const lw = window.innerWidth;
    const lh = window.innerHeight;
    // visual viewport размер
    const vw = vv ? vv.width  : window.innerWidth;
    const vh = vv ? vv.height : window.innerHeight;

    // Координаты для View Transition API (layout viewport)
    ...
    // Координаты для fallback overlay (visual viewport)
