(function () {
  ...
  /**
   * Центр иконки в координатах LAYOUT VIEWPORT.
   * 
   * getBoundingClientRect() даёт координаты относительно VISUAL VIEWPORT.
   * position:fixed, VT API и clip-path работают в LAYOUT VIEWPORT.
   * На мобильных они расходятся из-за UI-баров и pinch-zoom.
   * 
   * Преобразование: layout = visual * scale + offset
   */
  function getIconCenter() {
    const svg  = themeBtn?.querySelector('svg');
    const wrap = themeBtn?.querySelector('.icon-wrap');
    const el   = svg || wrap || themeBtn;
  ...
  async function toggle() {
    ...
    /* ── Fallback: transform:scale вместо clip-path ──
       clip-path + position:fixed на мобильных даёт смещение,
       поэтому анимируем круглый div через transform — это стабильно везде. */
    document.body.classList.add('no-tr');

    const size  = 10;                // начальный диаметр круга (px)
    const scale = (r * 2.2) / size;  // запас 10% по краям экрана

    if (nextDark) {
      // Темная тема расширяется из центра иконки
      ...
      requestAnimationFrame(() => requestAnimationFrame(() => {
        ov.style.transition = `transform ${dur}ms cubic-bezier(.4,0,.2,1)`;
        ov.style.transform  = `scale(${scale})`;
        ov.addEventListener('transitionend', () => { ov.remove(); busy = false; }, { once: true });
      }));
    } else {
      // Светлая тема: сужаем тёмный круг к центру иконки
      isDark = false;
      applyTheme(false);
      document.body.classList.remove('no-tr');
  ...
  // При загрузке сразу применяем сохранённую тему (без анимации)
