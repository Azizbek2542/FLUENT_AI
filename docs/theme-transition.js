(function () {
  ...
  /**
   * Центр SVG-иконки в двух системах координат:
   *
   * visual  → для position:fixed с left/top (Chrome Android fixed = visual viewport)
   * layout  → для ::view-transition-new(root) (root element = layout viewport)
   *
   * window.visualViewport — это JS-эквивалент dvh/dvw.
   * offsetTop/Left = на сколько visual viewport смещён от layout (высота адресной строки).
   */
  function getIconCenter() {
    ...
    if (!el) {
      return {
        visual: { x: window.innerWidth / 2, y: window.innerHeight / 2 },
        layout: { x: window.innerWidth / 2, y: window.innerHeight / 2 }
      };
    }
  ...
  async function toggle() {
    ...
    /* VT API = layout viewport (root/html element) */
    const lcx = c.layout.x;
    const lcy = c.layout.y;
    const lr  = maxR(lcx, lcy, layoutW, layoutH);
    const vtSmall = `circle(0px at ${lcx}px ${lcy}px)`;
    const vtBig   = `circle(${lr}px at ${lcx}px ${lcy}px)`;

    /* Fallback = visual viewport (position:fixed left/top) */
    const vcx = c.visual.x;
    const vcy = c.visual.y;
    const vr  = maxR(vcx, vcy, visualW, visualH);
    ...
    /* ── Fallback: круг через transform:scale в visual viewport ──
       НЕ используем clip-path на fullscreen overlay, потому что
       position:fixed + inset:0 = layout viewport размер,
       а координаты SVG — в visual viewport. Вместо этого
       позиционируем круг напрямую по visual coords и анимируем scale. */
    document.body.classList.add('no-tr');

    const startSize = 8;
    const scaleTo   = (vr * 2.2) / startSize;
    ...
    if (nextDark) {
      ...
      requestAnimationFrame(() => requestAnimationFrame(() => {
        ...
        circle.addEventListener('transitionend', () => { circle.remove(); busy = false; }, { once: true });
      }));
    } else {
      isDark = false;
      applyTheme(false);
